...
<div class="alert {{ $canInviteMore ? 'alert-light border' : 'alert-warning' }} d-flex justify-content-between align-items-center mb-3">
    <span>
        ...
        {{ $staffCount }} staff member(s) {{ $staffLimit ? "of {$staffLimit} allowed on your tier" : '(unlimited on your tier)' }}
    </span>
    @if(! $canInviteMore)
...
<div class="row">
    <div class="col-lg-4">
        <div class="card mb-3">
            <div class="card-header"><h5 class="card-title mb-0">Invite Staff</h5></div>
            <div class="card-body">
                <form action="{{ route('hotel.staff.invite') }}" method="POST">
                    @csrf
                    <div class="mb-3">
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header"><h5 class="card-title mb-0">Staff List</h5></div>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($member->is_active)
                                <form action="{{ route('hotel.staff.deactivate', $member->id) }}" method="POST" onsubmit="return confirm('Deactivate {{ $member->name }}?');">
                                    @csrf
                                    <button class="btn btn-sm btn-outline-danger">Deactivate</button>
                                </form>
                                @else
                                <form action="{{ route('hotel.staff.reactivate', $member->id) }}" method="POST">
                                    @csrf
